...
def _make_launch(context, *args, **kwargs):
    ...
    config_yaml = os.path.join(pkg_share, "config", "euroc", f"mono_{extractor}.yaml")
    weights_override = {"weights_folder": "weights/euroc"}
    ...
    if use_loop:
        loop_yaml = os.path.join(
            ...
            "config",
            "loop_fusion_euroc.yaml",
        )
        # cam0 calib is supplied via ROS param so loop_fusion doesn't need a
        # duplicate copy of cam0_pinhole.yaml inside its own config dir.
        cam0_calib_path = os.path.join(pkg_share, "config", "euroc", "cam0_pinhole.yaml")
        # Loop-closure mode.
