...
# Maps sequence name to its cam0 calib file (relative to pkg share/config/subt_mrs/).
# All visual-track sequences share the RC7 sensor rig (identical MEI calib).
_SEQ_CAM_CALIB = {
    "flash_light":    "cam_rc7_mei.yaml",
    "low_light1":     "cam_rc7_mei.yaml",
    "low_light2":     "cam_rc7_mei.yaml",
    "over_exposure":  "cam_rc7_mei.yaml",
    "smoke_handheld": "cam_rc7_mei.yaml",
}
...
def _make_launch(context, *args, **kwargs):
    ...
    if extractor not in VALID_EXTRACTORS_MONO:
        raise RuntimeError(
            f"extractor='{extractor}' not in VALID_EXTRACTORS_MONO {VALID_EXTRACTORS_MONO}"
        )
    if seq not in VALID_SEQUENCES:
        raise RuntimeError(
            f"seq='{seq}' not in VALID_SEQUENCES {VALID_SEQUENCES}"
        )

    pkg_share = FindPackageShare("dl_vins").perform(context)
    config_yaml = os.path.join(
        pkg_share, "config", "subt_mrs", f"mono_{extractor}.yaml")

    weights_override = {"weights_folder": "weights/subt_mrs"}
    ...
    composable_nodes = [
        ComposableNode(
            ...
            parameters=[config_yaml, weights_override, use_sim_time_param, viz_override],
            remappings=[
                ("imu",    "/imu/data"),
                ("image0", "/camera_1/image_raw"),
            ],
    ...
    if use_loop:
        loop_yaml = os.path.join(
            ...
            "config",
            "loop_fusion_subt_mrs.yaml",
        )
        cam0_calib_path = os.path.join(
            pkg_share, "config", "subt_mrs", _SEQ_CAM_CALIB[seq])
...
def generate_launch_description():
    return LaunchDescription([
        DeclareLaunchArgument(
            ...
            description=f"One of {VALID_EXTRACTORS_MONO}",
        ),
        DeclareLaunchArgument(
            "seq",
            default_value="flash_light",
            description=f"Sequence name; selects per-sequence calib. One of {VALID_SEQUENCES}",
        ),
        DeclareLaunchArgument(
            "use_loop_fusion",
            default_value="false",
