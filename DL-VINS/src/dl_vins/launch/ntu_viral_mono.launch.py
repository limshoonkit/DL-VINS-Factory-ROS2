...
def _make_launch(context, *args, **kwargs):
    ...
    config_yaml = os.path.join(pkg_share, "config", "ntu_viral", f"mono_{extractor}.yaml")
    weights_override = {"weights_folder": "weights/ntu_viral"}
    ...
    # NTU VIRAL convention: cam0 = right (matches camRight.yaml / /right/image_raw).
    composable_nodes = [
        ComposableNode(
            package="dl_vins",
    ...
    if use_loop:
        loop_yaml = os.path.join(
            ...
            "config",
            "loop_fusion_ntu_viral.yaml",
        )
        # cam0 calib is provided to loop_fusion via an absolute ROS param so we
        # don't have to duplicate camRight.yaml into loop_fusion/config/.
        cam0_calib_path = os.path.join(pkg_share, "config", "ntu_viral", "camRight.yaml")
        # Loop-closure mode.
