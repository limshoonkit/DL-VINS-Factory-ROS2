...
def _make_launch(context, *args, **kwargs):
    ...
    config_yaml = os.path.join(pkg_share, "config", "botanic_garden", f"mono_{extractor}_color.yaml")
    ...
    # Botanic Garden COLOR/RGB: cam0 = left (Dalsa color), cam1 = right.
    composable_nodes = [
        ComposableNode(
            package="dl_vins",
            ...
            remappings=[
                ("imu", "/imu/data"),
                ("image0", "/dalsa_color/left/image_raw"),
            ],
    ...
    if use_loop:
        ...
        cam0_calib_path = os.path.join(
            pkg_share, "config", "botanic_garden", "dalsa_rgb0_pinhole_down.yaml")
