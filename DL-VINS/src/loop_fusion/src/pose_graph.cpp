...
namespace
{
    inline double elapsedMs(const std::chrono::high_resolution_clock::time_point &start)
    {
...
void PoseGraph::startOptimization()
{
    printf("[loop_fusion] 4-DoF (x, y, z, yaw) pose-graph optimization thread starting\n");
    metrics_logger_.initLoopFusionLog();
    metrics_logger_.initLoopOptimizationLog();
...
void PoseGraph::addKeyFrame(KeyFrame *cur_kf, bool flag_detect_loop)
{
    ...
    if (loop_index != -1)
    {
        ...
        const bool connected =
            (cur_kf->has_loop && loop_index == old_kf->index) ||
            cur_kf->findConnection(old_kf);
        loop_closed = connected;
        if (connected)
        {
            ...
            Matrix3d shift_r;
            Vector3d shift_t;
            // 4-DoF (x, y, z, yaw): rotational correction is yaw-only.
            const double shift_yaw =
                Utility::R2ypr(w_R_cur).x() - Utility::R2ypr(vio_R_cur).x();
    ...
    // Emit per-keyframe loop-fusion metrics row. candidate_index/closed reflect
    // a candidate that passed BRIEF + PnP gates; top1_sim is the best retrieval
    // cosine regardless of threshold.
    if (metrics_logger_.isEnabled())
    {
        uosm::loop_fusion::LoopFusionMetrics m;
        m.timestamp = cur_kf->time_stamp;
        m.kf_index = cur_kf->index;
        m.candidate_index = loop_index;
        m.closed = loop_closed ? 1 : 0;
        m.num_inliers = cur_kf->last_inlier_count; // -1 if findConnection not called
        m.top1_sim = last_top1_sim_;               // threshold-independent best sim
        m.vpr_ms = cur_kf->vpr_ms;                 // DINO+VLAD inference time
        m.proc_ms = std::chrono::duration<double, std::milli>(
                        std::chrono::high_resolution_clock::now() - t_proc_start)
                        .count();
        metrics_logger_.logLoopFusion(m);
    }
}
...
int PoseGraph::detectLoop(KeyFrame *keyframe, int frame_index)
{
    ...
    const int max_frame_id_allowed = frame_index - RECALL_IGNORE_RECENT_COUNT;

    if (USE_DL_LOOP)
    {
        last_top1_sim_ = -1.0;
        if (frame_index < RECALL_IGNORE_RECENT_COUNT ||
            keyframe->dl_global_desc.empty())
            return -1;
        const std::vector<float> &q = keyframe->dl_global_desc;
        std::vector<std::pair<float, int>> cands; // (cosine similarity, kf index)
        float best_sim = -1.0f;                   // threshold-independent top1
        for (KeyFrame *kf : keyframelist)
        {
            if (kf->index >= max_frame_id_allowed ||
                kf->dl_global_desc.size() != q.size())
                continue;
            float sim = 0.0f; // VLAD descriptors are L2-normalised -> dot = cosine
            for (size_t k = 0; k < q.size(); ++k)
                sim += q[k] * kf->dl_global_desc[k];
            if (sim > best_sim)
                best_sim = sim;
            if (sim > DL_LOOP_SIM_THRESH)
                cands.emplace_back(sim, kf->index);
        }
        last_top1_sim_ = best_sim;
        if (cands.empty())
            return -1;
        // Most-similar first; first candidate that geometrically verifies wins.
        std::sort(cands.begin(), cands.end(),
                  [](const std::pair<float, int> &a, const std::pair<float, int> &b)
                  { return a.first > b.first; });
        const size_t max_try = std::min<size_t>(cands.size(), 10);
        for (size_t i = 0; i < max_try; ++i)
        {
            KeyFrame *old_kf = getKeyFrame(cands[i].second);
            if (old_kf && keyframe->findConnection(old_kf))
                return cands[i].second;
        }
        return -1;
    }
    ...
    QueryResults ret;
    db.query(keyframe->brief_descriptors, ret, 4, max_frame_id_allowed);
    ...
    // Try the oldest unverified candidate first; if PnP/BRIEF rejects it,
    // fall back to the next-oldest. Matches upstream pose_graph.cpp.
    ...
    while (done_ids.size() < ret.size())
    {
        ...
        for (unsigned int i = 0; i < ret.size(); i++)
        {
            if (ret[i].Id < min_index &&
                ret[i].Id < max_frame_id_allowed &&
                ret[i].Score > MIN_SCORE &&
                std::find(done_ids.begin(), done_ids.end(), ret[i].Id) == done_ids.end())
            {
                min_index = ret[i].Id;
                has_min = true;
            }
...
void PoseGraph::optimize4DoF()
{
    while (true)
    {
        ...
        if (cur_index != -1)
        {
            ...
            // Bound the optimized window to the most recent POSE_GRAPH_OPT_WINDOW
            // keyframes (0 = unbounded). The window never starts before the earliest
            // loop anchor, so a fixed gauge always exists.
            int window_start_index = first_looped_index;
            if (POSE_GRAPH_OPT_WINDOW > 0 &&
                cur_index - first_looped_index + 1 > POSE_GRAPH_OPT_WINDOW)
                window_start_index = cur_index - POSE_GRAPH_OPT_WINDOW + 1;

            int max_length = cur_index - window_start_index + 1;
            ...
            options.max_solver_time_in_seconds = POSE_GRAPH_MAX_SOLVER_TIME;
            options.max_num_iterations = POSE_GRAPH_MAX_ITERATIONS;
            options.num_threads = POSE_GRAPH_NUM_THREADS;
            ...
            ceres::Manifold *manifold = AngleLocalManifold::Create();

            list<KeyFrame *>::iterator it;

            int i = 0;
            int num_loops_in_problem = 0; // count keyframes with has_loop participating
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                if ((*it)->index < window_start_index)
                    continue;
                if ((*it)->has_loop)
                    ++num_loops_in_problem;
                (*it)->local_index = i;
                Quaterniond tmp_q;
                Matrix3d tmp_r;
                Vector3d tmp_t;
                ...
                problem.AddParameterBlock(euler_array[i], 1, manifold);
                problem.AddParameterBlock(t_array[i], 3);

                if ((*it)->index == window_start_index || (*it)->sequence == 0)
                {
                ...
                // Only add the loop edge if its connected keyframe is inside the
                // current optimization window
                if ((*it)->has_loop && (*it)->loop_index >= window_start_index)
                {
            ...
            const double optimize_ms = elapsedMs(t_opt_start);
            m_keyframelist.lock();
            i = 0;
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
            ...
            const double drift_xy = t_drift.head<2>().norm();
            m_drift.unlock();

            if (metrics_logger_.isEnabled())
            {
                uosm::loop_fusion::LoopOptimizationMetrics m;
                m.kf_index = cur_index;
                m.timestamp = cur_kf->time_stamp;
                m.optimize_ms = optimize_ms;
                m.drift_xy_m = drift_xy;
                m.drift_yaw_deg = yaw_drift;
                m.num_loops_optimized = num_loops_in_problem;
                metrics_logger_.logLoopOptimization(m);
            }
...
// optimize6DoF removed — DL-VINS port is IMU-only / 4DoF.
...
{
    m_keyframelist.lock();
    const auto t_save_start = std::chrono::high_resolution_clock::now();
    FILE *pFile;
