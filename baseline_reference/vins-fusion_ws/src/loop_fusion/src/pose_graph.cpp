...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
	...
	if (loop_index != -1)
	{
        // [DLVINS_DIAG] detected
        printf("[DLVINS_DIAG] %d detect loop with %d\n", cur_kf->index, loop_index);
        KeyFrame* old_kf = getKeyFrame(loop_index);

        const bool _dlvins_closed = cur_kf->findConnection(old_kf);
        printf("[DLVINS_DIAG] %d findConnection -> %s\n",
               cur_kf->index, _dlvins_closed ? "closed" : "rejected");
        if (_dlvins_closed)
        {
