                         ...
                         Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
    ...
    // RANSAC iterations 100 -> 200 to match DL-VINS loop_fusion (fair eval).
    if (CV_MAJOR_VERSION < 3)
        solvePnPRansac(matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 200, 10.0 / 460.0, 100, inliers);
    else
    {
        if (CV_MINOR_VERSION < 2)
            solvePnPRansac(matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 200, sqrt(10.0 / 460.0), 0.99, inliers);
        else
            solvePnPRansac(matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 200, 10.0 / 460.0, 0.99, inliers);
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	*/
	...
	double relative_yaw;
	// [DLVINS_DIAG] sizes after BRIEF Hamming match (pre-PnP)
	printf("[DLVINS_DIAG] kf=%d candidates=%d point_2d_uv=%d brief_window=%d brief_old=%d\n",
		   index, (int)matched_2d_cur.size(),
		   (int)point_2d_uv.size(), (int)window_brief_descriptors.size(),
		   (int)old_kf->brief_descriptors.size());
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    reduceVector(matched_3d, status);
	    reduceVector(matched_id, status);
	    printf("[DLVINS_DIAG] kf=%d after_pnp=%d\n", index, (int)matched_2d_cur.size());
	    #if 1
	    	if (DEBUG_IMAGE)
	        {
	        	int gap = 10;
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    // [DLVINS_DIAG] pose-gate inputs for candidates that cleared the inlier gate
	    printf("[DLVINS_DIAG] kf=%d posegate inliers=%d |t|=%.3f yaw=%.3f -> %s\n",
	           index, (int)matched_2d_cur.size(), relative_t.norm(), relative_yaw,
	           (abs(relative_yaw) < 30.0 && relative_t.norm() < 20.0) ? "PASS" : "FAIL");
