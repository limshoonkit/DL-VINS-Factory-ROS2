...
int main(int argc, char **argv)
{
...

    {
        ...
        if (!cv_T.empty())
        {
            qic << cv_T.at<double>(0, 0), cv_T.at<double>(0, 1), cv_T.at<double>(0, 2),
                   cv_T.at<double>(1, 0), cv_T.at<double>(1, 1), cv_T.at<double>(1, 2),
                   cv_T.at<double>(2, 0), cv_T.at<double>(2, 1), cv_T.at<double>(2, 2);
            tic << cv_T.at<double>(0, 3), cv_T.at<double>(1, 3), cv_T.at<double>(2, 3);
            printf("loop_fusion extrinsic from config body_T_cam0: tic=[%.4f %.4f %.4f]\n",
                   tic.x(), tic.y(), tic.z());
        }
        else
        {
            qic.setIdentity();
            tic.setZero();
            printf("WARN: body_T_cam0 missing in config; qic=I tic=0 (loop PnP may be biased)\n");
        }
    }
