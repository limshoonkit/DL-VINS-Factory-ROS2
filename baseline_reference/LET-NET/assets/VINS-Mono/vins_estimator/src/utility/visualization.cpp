...
void pubCameraPose(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        // write result to file for evaluation
        static std::vector<Eigen::Vector3d> pose_vec;
        static std::vector<double> timestamp_vec;
        static std::vector<Eigen::Quaterniond> quat_vec;
        pose_vec.push_back(P);
        timestamp_vec.push_back(header.stamp.toSec());
        quat_vec.push_back(R);

        ofstream foutC("/root/output/traj.txt", std::ios::out);
        for (int j = 0; j < pose_vec.size(); j++)
        {
            foutC.setf(ios::fixed, ios::floatfield);
            foutC.precision(0);
            foutC << timestamp_vec[j] * 1e9 << ",";
            foutC.precision(5);
            foutC << pose_vec[j].x() << ","
                  << pose_vec[j].y() << ","
                  << pose_vec[j].z() << ","
